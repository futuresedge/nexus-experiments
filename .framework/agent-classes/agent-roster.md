...
**Changes from 0.1:**
  - Added: ProjectRegistrar, EnvironmentContractAuthor,
    PatternSeeder, BootstrapValidator (all Zone 0, all MVP)
  - Zone 0 section added to summary table
  - Bootstrapping exception note added
...
**Summary table — Zone 0 additions:**

| Agent Class | Type | User-invokable | Track / Phase | MVP | Status |
|---|---|---|---|---|---|
| **ZONE 0** | | | | | |
| `FrameworkOwner` | OWNER | ✅ | Zone 0 / Learning Layer | ✅ | Defined |
| `ProjectRegistrar` | EXECUTOR | ❌ | Zone 0 — Z0-1 | ✅ | Needs class definition |
| `EnvironmentContractAuthor` | EXECUTOR | ❌ | Zone 0 — Z0-2 | ✅ | Needs class definition |
| `TemplateAuthor` | EXECUTOR | ❌ | Zone 0 — Z0-3 | ✅ | Needs class definition |
| `PatternSeeder` | EXECUTOR | ❌ | Zone 0 — Z0-4 | ✅ | Needs class definition |
| `BootstrapValidator` | REVIEWER | ❌ | Zone 0 — Z0-5 | ✅ | Needs class definition |
| `AgentSpecReviewer` | REVIEWER | ❌ | Zone 0 — Z0-3 | ✅ | Needs class definition |
| `UncertaintyOwner` | OWNER | ✅ | Cross-cutting | ✅ | Defined |
...
**Track / Phase:** Zone 0 — Task Z0-1
**MVP:** ✅

**Responsibility:**
Translates a human:director project brief into a structured,
machine-readable project manifest. Does not invent or assume
inputs — raises uncertainty for any ambiguous or missing required
field. The manifest is a faithful restatement of intent, not an
interpretation.

**Gate authority:** None — produces a draft; `human:director`
is the sole approver.

**READS:**
- Human input (freeform brief, passed as task context)
- `.framework/templates/project-manifest.template.md`

**WRITES:**
- `project-manifest.md` (one per project, Replace mode)

**NEVER:**
- `environment-contract.md`
- Any agent file
- Pattern Library
- Any lifecycle artefact

**Key skill:** `project-registration.skill.md`
Knows the required fields, knows what constitutes an ambiguous
input vs a missing input, knows how to surface open questions
without blocking on minor gaps.

**Certainty threshold:** 0.70
Must raise uncertainty freely — this agent works closest to
raw human input, where ambiguity is highest and invented values
are most dangerous.

**Reviewer:** `human:director`
(Not an agent reviewer — the human reviews their own intent,
restated. This is the Tier 4 gate for Zone 0.)
...
**Type:** EXECUTOR
**User-invokable:** false
**Track / Phase:** Zone 0 — Task Z0-2
**MVP:** ✅

**Responsibility:**
Writes a complete, internally consistent, and verifiable
environment contract from the project manifest's tech stack
declaration. Knows what fields are required per stack type
(static site, API, full-stack). Every field in the contract
must be independently verifiable — no aspirational or assumed
values.

**READS:**
- `project-manifest.md`
- `.framework/templates/environment-contract.template.md`

**WRITES:**
- `environment-contract.md` (one per project, Replace mode)
...
**Key skill:** `environment-contract-writing.skill.md`
Knows required contract fields per stack type, knows the
difference between a verifiable field ("build command:
`pnpm build`") and an unverifiable one ("fast build times"),
knows how to derive verification steps from tech stack
declarations.

**Certainty threshold:** 0.80

**Reviewer:** `EnvironmentReviewer`
First activation of `EnvironmentReviewer` — in contract
review mode, not post-deploy mode. Reviews for completeness,
internal consistency, and verifiability of every field.

**Open question:**
Should `EnvironmentContractAuthor` also handle contract
*updates* during the project lifecycle (e.g., when a new
service is added mid-project)? Proposed: yes, same agent,
same Replace-mode write. The contract is versioned in
document history. Deferred to post-bootstrap design.
...
**Type:** EXECUTOR
**User-invokable:** false
**Track / Phase:** Zone 0 — Task Z0-4
**MVP:** ✅

**Responsibility:**
Produces the initial Pattern Library entries for a declared
tech stack. Draws on tech stack knowledge embedded in its
skill file. Produces CANDIDATE-status patterns only —
promotion to PROVISIONAL is FrameworkOwner's gate, not
PatternSeeder's.

Does not invent patterns. Each entry must describe a real,
known-good approach with a plausible evidence basis.
Patterns that are too broad ("write clean code") or too
narrow ("only applies to this one task") are out of scope.
...
**WRITES:**
- Pattern Library entries (CANDIDATE status)
...
**Key skill:** `pattern-seeding.skill.md`
Knows the pattern schema, knows the distinction between
CANDIDATE / PROVISIONAL / ACTIVE evidence thresholds, knows
what makes a pattern genuinely reusable vs a project-specific
one-off. Contains tech-stack-specific pattern knowledge for
common stacks (AstroJS, Next.js, Express, etc.).

**Certainty threshold:** 0.75
Patterns are CANDIDATE only — lower certainty is acceptable
because FrameworkOwner reviews before promotion. Raising
uncertainty is appropriate for unfamiliar stacks.

**Reviewer:** `FrameworkOwner`
Reviews candidates for format conformance, plausibility of
evidence basis, and reusability scope. Promotes to
PROVISIONAL or returns with findings.
...
**Type:** REVIEWER
**User-invokable:** false
**Track / Phase:** Zone 0 — Task Z0-5
**MVP:** ✅

**Responsibility:**
Verifies that Zone 0 is complete and the Nexus Framework is
operational for this project. Does not check that files exist —
checks that outputs are functional. Runs the bootstrap smoke
test (creates a test task, walks it to `CONTEXT_READY`,
cancels it, verifies the audit log). Writes a bootstrap
report with pass/fail evidence for every checklist item.

**READS:**
- `project-manifest.md`
- `environment-contract.md`
- All generated `.agent.md` files
- Pattern Library entries
- Bootstrap completion checklist
- Audit log (smoke test verification)

**WRITES:**
- `bootstrap-report.md` (Create-once)

**NEVER:**
- Any Zone 0 source document (reads only)
- `project-manifest.md`
- `environment-contract.md`
- Any agent file (reads to verify, never writes)

**Key skill:** `bootstrap-validation.skill.md`
Knows the bootstrap completion checklist, knows how to run
the smoke test sequence, knows the difference between a file
existing and a system being operational, knows how to
interpret audit log entries as evidence of correct behaviour.

**Certainty threshold:** 0.90
BootstrapValidator's PASS verdict is the final gate before
product work begins. High certainty required — a false
positive here corrupts all downstream work.

**Reviewer:** `human:director`
Final human gate for Zone 0. Reviews bootstrap report for
completeness. Approves or returns with gaps to address before
any Delivery work begins.
...
| 0.2 | 2026-03-05 | Added Zone 0 agent classes: ProjectRegistrar,
|     |            | EnvironmentContractAuthor, PatternSeeder,
|     |            | BootstrapValidator. Zone 0 section added to
|     |            | summary table. Bootstrapping exception note
|     |            | updated. |